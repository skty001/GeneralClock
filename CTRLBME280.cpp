...
// BME280から温度・湿度・気圧を取得
void CTRLBME280::getData(double &temp, double &hum, double &pre){
  int i = 0;
  ...
  Wire.requestFrom((int)BME280_ADDRESS, 8);    // 受信開始
  ...
  temp = (double)comp_temp / 100;          // 分解能0.01 Ex.5123→51.23℃
  hum = (double)comp_hum / 1024;           // Q22.10形式32Bit整数(22bit整数と10bit小数)
  pre = (double)comp_press / 256 / 100;  // Q24.8形式32Bit整数(24bit整数と8bit小数) (Pa→hPa)
}
...
void CTRLBME280::writeRegister(unsigned char addr, unsigned char data) {
...
// BME280上の調整パラメータを取得
void CTRLBME280::readCalibration(){
  int i = 0;
  ...
  Wire.write(0x88);
  Wire.endTransmission();                 // 送信完了
  Wire.requestFrom((int)BME280_ADDRESS, 25);   // 0x88→0xA1
  ...
  Wire.write(0xE1);
  Wire.endTransmission();                 // 送信完了
  Wire.requestFrom((int)BME280_ADDRESS, 7);    // 0xE1→0xE8
  while(Wire.available()){
    data[i] = Wire.read();
    i++;
  }
...
long signed int CTRLBME280::BME280_compensate_T_int32(long signed int adc_T) {
...
long unsigned int CTRLBME280::BME280_compensate_P_int64(long unsigned int adc_P)
{
...
long unsigned int CTRLBME280::BME280_compensate_H_int32(long signed int adc_H)
{
